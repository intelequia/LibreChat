...
function transformResultToMCPToolCallResponse(result: any): MCPToolCallResponse {
  return {
    ...
    content: result.content?.map((item: any) => {
      if (item.type === 'text') {
        return { type: 'text', text: item.text } as ToolContentPart;
      } else if (item.type === 'image') {
        return { type: 'image', data: item.data, mimeType: item.mimeType } as ToolContentPart;
      } else if (item.type === 'audio') {
        return { type: 'audio', data: item.data, mimeType: item.mimeType } as ToolContentPart;
      }
...
export class MCPManager {
  ...
  async callTool({
    ...
    try {
      ...
      const transformedResult = transformResultToMCPToolCallResponse(result);
      if (userId) {
        this.updateUserLastActivity(userId);
      }
      this.checkIdleConnections();
      return formatToolContent(transformedResult, provider);
    } catch (error) {
