...
let configuredPricingMethods;
let configuredFindUser;
const fallbackPricingMethods = {
  getMultiplier: () => 1,
  getCacheMultiplier: () => null,
};

const configureAppInsights = ({ pricing, findUser } = {}) => {
  configuredPricingMethods = pricing;
  configuredFindUser = findUser;
};

const resolveUser = async (userId) => {
  if (configuredFindUser) {
    return configuredFindUser({ _id: userId });
  }

  const userModel = mongoose.models.User;
  return userModel ? userModel.findById(userId).select('email').lean() : null;
};
...
const enrichPropertiesWithUserEmail = async (properties) => {
  ...
  try {
    const user = await resolveUser(properties.userId);
    if (user && user.email) {
...
const getCacheReadTokens = (usage = {}) =>
  firstTokenCount(usage.input_token_details?.cache_read, usage.cache_read_input_tokens);
...
const getPricing = (pricing) => pricing || configuredPricingMethods || fallbackPricingMethods;
...
const getUsageRates = ({
  ...
  const promptRate = toRate(
    pricingMethods.getMultiplier({
      model,
      endpoint,
      tokenType: 'prompt',
      inputTokenCount,
...
const computeRatedUsage = (tokenUsage = {}, options = {}) => {
  ...
  const rawMessageTokens =
    rawPromptTokens + rawCacheCreationTokens + rawCacheReadTokens + rawCompletionTokens;
  const inputTokenCount =
    options.inputTokenCount ?? rawPromptTokens + rawCacheCreationTokens + rawCacheReadTokens;
  ...
  const rawInputTokens =
    tokenUsage.rawInputTokens == null
...
const aggregateCollectedUsage = (collectedUsage, options = {}) => {
  ...
  return {
    ...
    promptRate: weightedRate(
      totals.ratedPromptTokens,
      totals.rawPromptTokens,
      firstRates.promptRate,
    ),
...
const trackTokenUsage = async (txData, tokenUsage, eventType, additionalData = {}) => {
  ...
  try {
    ...
    if ((configuredFindUser || mongoose.models.User) && !user) {
      return;
    }
    ...
    if (inputTokens > 0 || cacheCreationTokens > 0 || cacheReadTokens > 0 || completionTokens > 0) {
      ...
      if (shouldTrack) {
        await trackTokenUsage(
          txData,
          { promptTokens: inputTokens, cacheCreationTokens, cacheReadTokens, completionTokens },
          eventType,
          {
            endpoint: trackingData.endpoint || txData.endpoint,
            model: trackingData.model || txData.model,
