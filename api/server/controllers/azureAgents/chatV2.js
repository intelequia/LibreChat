...
const { runAssistant, createOnTextProgress } = require('~/server/services/AzureAgentService');
...
const chatV2 = async (req, res) => {
  logger.debug('[/azureAgents/chat/] req.body', req.body);
  ...
  /** @type {AIProjectsClient} */
  let azureAgentClient;
  ...
  try {
    ...
    const initializeThread = async () => {
      ...
      const result = await initAzureAgentThread({ azureAgentClient, body: initThreadBody, thread_id });
    ...
    const processRun = async (retry = false) => {
      ...
      if (retry) {
        response = await runAssistant({
          ...
          thread_id,
          run_id,
          in_progress: azureAgentClient.in_progress,
        });
        return;
      }
      ...
      /* NOTE:
        ...
        */
      const run = await createAzureAgentRun({
        azureAgentClient,
        thread_id,
        body,
      });
